...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; the assistant handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    ...
    "For behavior, commands, config, or architecture: consult local docs first.",
    "",
  ].filter(Boolean);
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const lines = [
    ...
    ...safetySection,
    "## CLI Quick Reference",
    "The assistant is controlled via subcommands. Do not invent commands.",
    ...
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, the assistant pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
  ...
  if (!isMinimal && heartbeatPrompt) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'A leading/trailing "HEARTBEAT_OK" is treated as a heartbeat ack (and may be discarded).',
...
export function buildRuntimeLine(
  ...
  // Privacy: omit host, os, arch, node, repoRoot, model, and branding
  // to prevent leaking personal/system info to LLM providers.
